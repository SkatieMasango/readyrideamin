            ...
            <div class="card-body p-6">
                <div class="flex items-center justify-between">
                    <h5 class="text-lg font-semibold text-gray-800">
                        Marketing
                        <span class="text-sm text-gray-500">List of all gift cards.</span>
                    </h5>
                    <div class="flex items-center gap-3">
                        ...
                        <x-secondary-button type="button" x-data
                            @click="window.location.href = '{{ route('marketing-gift-cards.create') }}'">
                            {{ __('Add Gift Card ') }}
                        </x-secondary-button>
                    </div>
                </div>
                    ...
                    <div class="card-body p-3">
                        <div class="flex flex-direction items-end lg:items-center justify-end gap-3">
                            <div class=" w-half max-w-300 ">
                                <form action="{{ route('marketing-gift-cards.index') }}"
                                    class="relative tp-driver-form">
                                    <div class="flex w-full">
                                        <input type="text" name="search" value="{{ request('search') }}"
                                            class=" w-full " placeholder="Search gift card">
                                        <x-search></x-search>
                                    </div>
                                </form>
                            </div>
                        ...
                        <tbody class="text-sm text-gray-700">
                            @foreach ($data as $giftcard)
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="px-4 py-3">{{ $giftcard->created_at }}</td>
                                    <td class="px-4 py-3"><strong>{{ $giftcard->title }}</strong></td>
                                    <td class="px-4 py-3"> {{ $giftcard->code }}</td>
                                    <td class="px-4 py-3"> {{ $giftcard->max_users }}</td>
                                    <td class="px-4 py-3">
                                        @php
                                            $riders = \App\Models\Rider::whereIn(
                                                'id',
                                                $giftcard->rider_ids ?? [],
                                            )->get();
                                        @endphp
                                        @foreach ($riders as $rider)
                                            {{ $rider->user->name ?? 'N/A' }}@if (!$loop->last)
                                                ,
                                            @endif
                                        @endforeach
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        {{ \Carbon\Carbon::parse($giftcard->valid_from)->format('Y-m-d') }} to
                                        {{ \Carbon\Carbon::parse($giftcard->valid_to)->format('Y-m-d') }}</td>
                                    <td class="px-4 py-3">
                                        {{ $giftcard->discount_percent . '% ' . '(' . $giftcard->discount_flat . ' flat' . ')' }}
                                    </td>
                                    <td class="px-4 py-3"><span
                                            class="font-medium text-yellow-600">{{ $giftcard->is_enabled == 1 ? 'Enabled' : 'Disabled' }}</span>
                                    </td>
                                    <td class="px-4 py-3"><span
                                            class="font-medium text-yellow-600">{{ $giftcard->is_notified == 1 ? 'Notified' : 'Not Notified' }}</span>
                                    </td>
                                    <td class=" tp-btn-action ">
                                        <div class="tp-order d-flex align-items-center justify-content-center relative">
                                            ...
                                            <div class="tp-order-thumb-more absolute">
                                                <a type="submit"
                                                    href="{{ route('marketing-gift-cards.edit', $giftcard->id) }}"
                                                        ...
                                                        <div class="bg-white rounded-lg shadow-lg p-6 w-[90%] max-w-md">
                                                            <div class="mb-4 text-center">
                                                                ...
                                                                <p class="text-gray-700 mt-2 text-md">
                                                                    You want to permanently delete this giftcard?
                                                                </p>
                                                            </div>
                                                            ...
                                                            <div class="flex justify-center gap-3">
                                                                ...
                                                                <form method="POST"
                                                                    action="{{ route('marketing-gift-cards.destroy', $giftcard->id) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
