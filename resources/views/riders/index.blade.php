            ...
            <div class="card-body p-6 ">
                <div class="flex items-center justify-between" x-data="{ openModal: false }">
                    <h5 class="text-lg font-semibold text-gray-800">
                        Riders
                        <span class="text-sm text-gray-500">List of all riders.</span>
                    </h5>
                    ...
                    <div x-show="openModal" x-cloak
                        ...
                        x-transition>
                        <div class="bg-white p-6 rounded-lg shadow-lg create-rider modalContent" style="width:25rem">
                            <div class="flex justify-between items-center pb-2">
                                <h2 class="text-lg font-semibold">Add new rider</h2>
                                <button @click="openModal = false" class="text-4xl text-gray-500 hover:text-gray-700">
                                    &times;
                                </button>
                            </div>
                            <form id="form" method="POST" action="{{ route('riders.store') }}">
                                @csrf
                    ...
                    <div class="card-body p-3">
                        <div class="flex flex-direction items-end lg:items-center justify-end gap-3">
                            <div class=" w-full max-w-300 ">
                                <form action="{{ route('riders.index') }}" class="relative tp-rider-form">
                            ...
                            <div class="flex justify-between gap-3">
                                <div x-data="{ openExportModal: false }" @close-all.window="openExportModal = false">
                                    ...
                                    <div x-show="openExportModal" x-cloak x-transition
                                        ...
                                        <a href="{{ route('riders.export', array_merge(request()->query(), ['export_type' => 'pdf'])) }}"
                                            class="w-full text-left text-sm text-gray-700 hover:bg-gray-100 flex items-center justify-center p-1 gap-1">
                                            <span>PDF</span>
                                        </a>

                                        <a href="{{ route('riders.export', array_merge(request()->query(), ['export_type' => 'csv'])) }}"
                                            class="w-full text-left text-sm text-gray-700 hover:bg-gray-100 flex items-center justify-center p-1 gap-1">
                                            <span>CSV</span>
                                        </a>
                                    </div>
                                ...
                                <div x-show="openFilterModal" x-cloak x-transition @click.away="openFilterModal = false"
                                    ...
                                    <!-- Filter Form -->
                                    <form method="GET" action="{{ route('riders.index') }}">
                                        <div>
                        ...
                        <tbody class="text-sm text-gray-700">
                            @foreach ($data as $rider)
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="px-4 py-3">#{{ $rider->id ?? '' }}
                                    </td>
                                    <td class="px-4 py-3">{{ $rider->user?->mobile ?? '' }}
                                    </td>
                                    <td class="px-4 py-3"><strong>{{ $rider->user?->name ?? '' }}
                                        </strong><br>
                                        <span class="text-sm text-gray-500">Registered on
                                            {{ \Carbon\Carbon::parse($rider->created_at)->diffForHumans() }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">{{ $rider->user?->address ?? 'N/A' }}
                                    </td>
                                    <td class="px-4 py-3">{{ count($rider->orders ?? []) }}</td>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center">
                                            <span class="font-medium ">{{ $rider->user?->status?->label() ?? 'N/A' }}</span>
                                        </div>
                                    </td>
                                    <td class=" tp-btn-action ">
                                        <div class="tp-order d-flex align-items-center justify-content-center relative">
                                            ...
                                            <div class="tp-order-thumb-more absolute">
                                                <a type="submit" href="{{ route('riders.edit', $rider['id']) }}"
                                                        ...
                                                        <div
                                                            ...
                                                            <div class="mb-4 text-center">
                                                                ...
                                                                <p class="text-gray-700 mt-2 text-md">
                                                                    You want to permanently delete this rider?
                                                                </p>
                                                            </div>
                                                            <div class="flex justify-center gap-3">
                                                                ...
                                                                <form method="POST"
                                                                    action="{{ route('riders.destroy', $rider->id) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
